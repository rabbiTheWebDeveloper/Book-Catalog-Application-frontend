...
const customTheme = {
  dropdown: {
    ...
    floating: {
      ...
      header:
        "block py-2 px-4 text-sm text-gray-700 dark:text-gray-200   bg-red-200 w-[400px]",
      ...
      target:
        " h-8 w-30 px-3 py-2 rounded-3xl text-lg bg-transparent text-[#000] hover:bg-[#000] text-[#000]  hover:text-white   border border-[#000] text-center ",
    },
    inlineWrapper: "",
  },
};
    ...
    <Flowbite theme={{ theme: customTheme }}>
      <Dropdown label={header_label}>
        {items?.map((item , index) => (
          <Dropdown.Item key={index}  onClick={() => handleFilterValue(item)}>
            {item}
          </Dropdown.Item>
        ))}
      </Dropdown>
